  ...
  <div class="app-root">
    <header class="top-bar">
      ...
      <div class="toolbar">
        ...
        <el-button size="small" type="warning" @click="store.detect()" :disabled="!store.result" :loading="store.loading">⚠ 检测异常</el-button>
        <el-tag v-if="store.result" size="small" type="success" class="snap-tag">
          当前结果: {{ store.result.logType }} · {{ store.result.totalLogs }} 条
          <span v-if="store.result.logType !== store.logType">（切换类型后旧结果已失效）</span>
        </el-tag>
        <el-tag v-else size="small" type="info" class="snap-tag">未生成结果</el-tag>
      </div>
    </header>
    <div class="main-grid">
      <div class="grid-col">
